...
def unsigmoid(y, eps=1e-6):
    y = y.clip(min=eps, max=1-eps)
    return (y/(1-y)).log()

def cal_sim(x, y, tau=1, eps=1e-4):
    if y.dim() == 2:
...
def visualize_heat(x, file_path='tmp.jpg'):
    if isinstance(x, torch.Tensor):
        x = x.detach().cpu().numpy()

    if len(x.shape) == 2:
        x = x[..., None]
    vis = cv2.applyColorMap((x*255).astype(np.uint8), cv2.COLORMAP_JET)
...
def masks_to_boxes(masks):
    ...
    x_mask = ((masks>th) * x.unsqueeze(0))
    x_max = x_mask.flatten(1).max(-1)[0]
    x_min = x_mask.masked_fill(~(masks>th), fillvalue).flatten(1).min(-1)[0]
    x_min[x_min==fillvalue] = 0

    y_mask = ((masks>th) * y.unsqueeze(0))
    y_max = y_mask.flatten(1).max(-1)[0]
    y_min = y_mask.masked_fill(~(masks>th), fillvalue).flatten(1).min(-1)[0]
    y_min[y_min==fillvalue] = 0
...
def masks_noise(masks, apply_incoherent=True):
    ...
    if apply_incoherent :
        ...
        mask_noise = (mask_noise - 0.5) * 2 #(-1, 1)
        gt_masks_vector = ((gt_masks_vector + mask_noise) > 0.5).float()
